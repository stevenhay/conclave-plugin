...
public class MarePlugin : MediatorSubscriberBase, IHostedService
{
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        Logger.LogInformation("Launching {name} {major}.{minor}.{build}", "Conclave", version.Major, version.Minor, version.Build);
        Mediator.Publish(new EventMessage(new Services.Events.Event(nameof(MarePlugin), Services.Events.EventSeverity.Informational,
            $"Starting Conclave {version.Major}.{version.Minor}.{version.Build}")));
    ...
    private async Task WaitForPlayerAndLaunchCharacterManager()
    {
        ...
        try
        {
            ...
            {
                Logger?.LogDebug("--------- SWITCH TO INTRO UI MESSAGE -------");
